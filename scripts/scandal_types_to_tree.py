...
cur.execute('SELECT id, name, parent_id FROM scandal_subtypes')
for st in cur.fetchall():
    # copy subtype to types table (retrieve the new id)
    cur.execute('INSERT INTO scandal_types (name, parent) VALUES (%s, %s) RETURNING id',\
        (st['name'], st['parent_id']))
    new_id = cur.fetchone()['id']
    # change subtype_id to new one in scandals table
    cur.execute('UPDATE scandals SET subtype_id = %s WHERE subtype_id = %s', (new_id, st['id']))

# for each row in scandals
cur.execute('SELECT id, type_id, subtype_id, types FROM scandals')
for sc in cur.fetchall():
    ...
    cur.execute('UPDATE scandals SET types = %s WHERE id = %s',\
        (types, sc['id']))
